...
// Spec Option, but we can't make Go's type system play nice with
...
// Displayable property in a Spec declaration and be dropped in
// here by reference.
...
func Transition(c ui.Context, option ControlOptionAssigner,
	...
	finish interface{},
	durationMs int,
	easingFunc EasingFunc) ui.Option {
	...
	var unsub events.Unsubscriber

	var update = func(d ui.Displayable) {
	...
	return func(d ui.Displayable) {
		...
		unsub = d.Context().OnFrameEntered(func(e events.Event) {
			update(d)
		})
