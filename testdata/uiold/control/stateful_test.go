...
func TestState(t *testing.T) {
	...
	t.Run("Spec applies options", func(t *testing.T) {
		box := Box(context.New(),
			OnState("default", Width(100)),
	...
	t.Run("undefined default state reruns all initial options", func(t *testing.T) {
		...
		assert.Equal(root.Width(), 10, "Default state")
		root.SetState("wider")
		root.Context().Builder().Update(root)
		assert.Equal(root.Width(), 20, "wide state")
		root.SetState("default")
		root.Context().Builder().Update(root)
		assert.Equal(root.Width(), 10)
	})
}
