...
func Define(typeName string, constr interface{}, specOpts ...Option) DisplayableDefinition {
	return func(c Context, instanceOpts ...Option) Displayable {
		...
		if c == nil {
			panic("comp.Define() requires a Context as first argument, try Spec(context.New()) or in the parent closure, add a (b Builder) argument and forward it to the child nodes")
		}
