...
func App(model *TodoAppModel, options ...spec.Option) *AppSpec {
	app := &AppSpec{}
	spec.Apply(app, options...)
	return app
}
...
func Todo(model *TodoItemModel) *TodoSpec {
	todo := &TodoSpec{}
	/*
		spec.Apply(todo,
			spec.Child(HBox(
				spec.Child(Checkbox(Selected(!model.CompletedAt.IsZero(), Text(model.Text)))),
				spec.Child(Button(Text("X"), OnClick(model.Complete()))),
			)))
	*/
