...
func TestDisplayable(t *testing.T) {
	t.Run("Traits", func(t *testing.T) {
		box := Box(ctx.New(), TraitNames("abcd", "efgh"))
	...
	t.Run("PushUnsub", func(t *testing.T) {
		...
		root := Box(ctx.New(), On("foo", handler), On("foo", handler))
	...
	t.Run("Text", func(t *testing.T) {
		root := Box(ctx.New(), Text("abcd"))
		assert.Equal(t, root.Text(), "abcd")
	})

	t.Run("Title", func(t *testing.T) {
		root := Box(ctx.New(), Title("abcd"))
