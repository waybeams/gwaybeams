...
func TestCoordToComponent(t *testing.T) {
	...
	t.Run("CoordToComponent", func(t *testing.T) {
		...
		t.Run("Returns element within bounds", func(t *testing.T) {
			root := createTree()
			result := comp.CoordToComponent(root, 15, 15)
		...
		t.Run("Returns element on first pixel", func(t *testing.T) {
			root := createTree()
			result := comp.CoordToComponent(root, 10, 10)
			assert.NotNil(t, result)
		...
		t.Run("Returns element on last pixel", func(t *testing.T) {
			root := createTree()
			result := comp.CoordToComponent(root, 80, 20)
			assert.NotNil(t, result)
			assert.Equal(t, result.ID(), "abcd")
		})
		...
		t.Run("Returns next element", func(t *testing.T) {
			root := createTree()
			result := comp.CoordToComponent(root, 20, 35)
			assert.NotNil(t, result)
		...
		t.Run("Only returns Focusable elements", func(t *testing.T) {
			root := createTree()
			result := comp.CoordToComponent(root, 20, 40)
			assert.NotNil(t, result)
	...
	t.Run("LocalToGlobal", func(t *testing.T) {
		t.Run("No parent", func(t *testing.T) {
			instance := Button(context.New(), Width(100), Height(100))
			x, y := comp.LocalToGlobal(instance, 50, 60)
			assert.Equal(t, x, 50)
			assert.Equal(t, y, 60)
		})
		...
		t.Run("Single parent", func(t *testing.T) {
			...
			x, y := comp.LocalToGlobal(abcd, 20, 30)
			assert.Equal(t, x, 30)
			assert.Equal(t, y, 40)
		})
		...
		t.Run("Nested Parents", func(t *testing.T) {
			...
			x, y := comp.LocalToGlobal(root, 20, 30)
			assert.Equal(t, x, 20)
			assert.Equal(t, y, 30)
			...
			x, y = comp.LocalToGlobal(efgh, 20, 30)
			assert.Equal(t, x, 40)
			assert.Equal(t, y, 50)

			ijkl := root.FindComponentByID("ijkl")
			x, y = comp.LocalToGlobal(ijkl, 20, 30)
			assert.Equal(t, x, 50)
			assert.Equal(t, y, 60)

			mnop := root.FindComponentByID("mnop")
			x, y = comp.LocalToGlobal(mnop, 20, 30)
			assert.Equal(t, x, 60)
			assert.Equal(t, y, 70)
		})
	})
